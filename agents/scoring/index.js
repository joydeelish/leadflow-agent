...
app.post("/score", gateway.require("$0.01"), async (req, res) => {
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are a lead qualification expert for real estate businesses in Nigeria. Score leads from 0 to 100 based on likelihood to convert to a property purchase. Consider: budget signals, company size, location, engagement, and intent. Return ONLY valid JSON: { score: number, reason: string, tier: hot|warm|cold }"
      },
      {
        role: "user",
        content: "Score this lead: " + JSON.stringify(enrichedLead, null, 2)
      }
    ],
